...
def parseArgs():
    ...
    parser.add_argument('--xstub', nargs='+', dest='xstub',
            help='xstub min max and step')
...
#------- edited for xstub
   # xStub(s)
    if parsed_args.xstub==None:
        raise RuntimeError('Must specify xstub to process')
    else:
        xstubs = getXstub(parsed_args.xstub)
#--------edited for xstub
    ...
    for run in runs:
        ...
        if not os.path.isfile(coda):
            ...
            continue
        for stub in xstubs:
            job['name'] =  wf_name + '_' + coda_stem +'_stub' + str(stub) 
            job['input'] = [{}]
            ...
            job['command'] = " ".join([batch, replay_script, str(run), str(evts), str(stub)])
...
def getReplayRuns(run_args):
    ...
    return runs
#------------------------------------------------------------------------------
def getXstub(xstub_args):
    xstubs=[]
    for arg in xstub_args:
        print(arg)
        if re.match('^\d.\d+,\d\d.\d+,\d.\d+$', arg): # ^begining of the string, $ end of the string
            limits = re.split(r',', arg)
            min1 = float(limits[0])
            max1 = float(limits[1])
            step1 = float(limits[2])
            print(min1)
            print(max1)
            print(step1)
            #for xstub in range(min1,max1,step1):    #range cannot be used for integers
            #    xstubs.append(arg)
            for i in numpy.arange(min1,max1,step1):  #numpy.arange is one way of handling float in range 
                print(i) 
                xstubs.append(i)   
            print(xstubs)    
